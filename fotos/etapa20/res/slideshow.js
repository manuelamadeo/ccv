// File slideshow.js for the Slide Show 4 skin version 3.2.4 last change: 2015-12-14 19:18
var embedded = false;	
var pause = false;
var closing = false;
...
if (((window.location.hash.length) > 0)) { 
  ...
  if (iPhone) { 
    ...
    $('#theViewport').attr('content','width=device-width,initial-scale=1.0, user-scalable=yno') // if you set it to yes you can zoom by pinching, but with strange effects
  }
...
//	$('body').css({'overflow-x': 'hidden' }); 
	$("#slideDiv").swipe("destroy");  // was #image
	$("#slideDiv").swipe( {
...
*/
  ...
  if (typeof(imgThemeMinScreenHeight) != "undefined") {
    if (screenHeight < parseInt(imgThemeMinScreenHeight)) {
		//$(".themeImage img").hide();
		 if ($("h1").length <= 0){ 
			  $(".themeImage").prepend('<h1>' + $("title").text() + '</h1>');
		} else { $("h1").removeAttr('id');}
		$("h1").css('color',$("body").css('color'));		
	} else $(".themeImage img").show();
  } else $(".themeImage img").show();
